...
N = 1000;
noise_l = 0.1*randn(N,1);
noise_r = 0.1*randn(N,1); 
y_l = zeros(N,1);
y_r = zeros(N,1);
...
for t=2:N
  ...
  if(!check_stability_var(theta_trajectory_recu(:,t)) && t > 50) 
    ...
    theta_trajectory_recu(:,t) = ...
        wwr_estimation(min([ewls_equivalent_window_length, t]), ...
        model_output(:,t-(min([ewls_equivalent_window_length, t]))+1:t), ...
        noise_variance_trajectory_recu(:,:,t-1));
...
for t = 2:N
  ...
  for i=1:2:2*model_rank
     A(1:2,i:i+1) = [...
                          theta_trajectory_recu( (i):(i+1),t )'; ...
                          theta_trajectory_recu( (2*model_rank+i):(2*model_rank+i+1),t )' ...
                         ];
  endfor
