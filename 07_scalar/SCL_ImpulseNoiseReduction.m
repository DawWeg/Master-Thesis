  ...
  while(t <= N-max_corrupted_block_length-model_rank);
    ...
    if(use_external_detection)
      if(detection_signal(t) == 1 && detection_signal(t-1) == 0 && skip_detection <= 0)
        ...
        ewls_for_interpolation = ewls_coefficients_estimate(:,t-1);
        
        %%% Stability check
        if(!check_stability(ewls_coefficients_estimate(:,t-1), model_rank)) 
        printf("Model unstable on: %d.\n", t);          
        ewls_for_interpolation = levinson_durbin_estimation( ...
          min([ewls_equivalent_window_length, t-1]), ...
          input_signal(t-(min([ewls_equivalent_window_length, t-1])):t-1));    
        endif
