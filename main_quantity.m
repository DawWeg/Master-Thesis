...
%input_filename = filenames(3 ,:); seconds_start = 0; seconds_end = 1.5;
...
figure(1);
subplot(2,1,1); plot(c(1,range)); xlim([-inf inf]); hold on;
plot(noisy_signal'(1,range)); xlim([-inf inf]); hold off;
subplot(2,1,2); plot(c(2,range)); xlim([-inf inf]); hold on;
plot(noisy_signal'(2,range)); xlim([-inf inf]); hold off;
